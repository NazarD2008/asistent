...
def find_content(query: str = "") -> str:
    """Знаходить сторінку фільму/серіалу через звичайний Google-пошук.

    Важливо: URL будується з реального пошукового запиту, а не з
    підставленого site:-фільтра. Це залишає Google відповідальним за
    пошук актуальних результатів та дозволяє користувачу вибрати сервіс.
    """
    query = str(query or "").strip()
    if not query:
        return "Не вказано назву контенту."

    # Один універсальний запит для будь-якого фільму, серіалу, шоу, аніме.
    google_query = f'"{query}" де подивитися Україна'
    ...
    try:
        ...
        print(f"[browser] Google: {google_url}")
