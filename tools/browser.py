...
def find_content(query: str = "") -> str:
    """Шукає сторінку фільму/серіалу/шоу через Google без хардкоду тайтлів."""
    query = str(query or "").strip()
    if not query:
        return "Не вказано назву контенту."

    # Пошук саме назви + наміру користувача. Google сам підбирає
    # сторінки сервісів, каталоги та офіційні джерела.
